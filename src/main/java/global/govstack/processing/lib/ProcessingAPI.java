...
public class ProcessingAPI extends ApiPluginAbstract implements PropertyEditable {
    ...
    protected ApiRequestProcessor createRequestProcessor(String serviceId) {
        ...
        if ("true".equalsIgnoreCase(useGovStack)) {
            ...
            try {
                // Use V2 service that handles multiple forms
                LogUtil.info(CLASS_NAME, "Using GovStackRegistrationServiceV2 for service: " + configuredServiceId);

                GovStackRegistrationServiceV2 govStackService = new GovStackRegistrationServiceV2(configuredServiceId);
                // Validate that the request service ID matches configured service ID
