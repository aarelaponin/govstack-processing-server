...
import global.govstack.processing.exception.ValidationException;
...
public class RegistrationService implements ApiRequestProcessor {
    ...
    public JSONObject processApplication(String requestBody)
        ...
        String farmerUsername = configValues.get(Constants.FARMER_USERNAME);
        ...
        String submittedFormId = submitFormData(requestBody, formId, configJsonStr, farmerUsername, adminUsername);

        // Step 4: Start workflow process
        String processId = startWorkflowProcess(processDefId, submittedFormId, farmerUsername, adminUsername);

        // Step 5: Process submit activity
        processSubmitActivity(processId, processDefId, submitActivityId, farmerUsername, adminUsername, submittedFormId);
    ...
    private String submitFormData(String requestBody, String formId, String config,
                                  String farmerUsername, String adminUsername)
            throws FormSubmissionException, ValidationException {

        try {
            // Execute as farmer with proper user context management
            return UserContextUtil.executeAsUser(workflowUserManager, farmerUsername, adminUsername, () -> {
                try {
    ...
    private void processSubmitActivity(String processId, String processDefId, String submitActivityId,
                                       String farmerUsername, String adminUsername, String submittedFormId) {

        UserContextUtil.executeAsUser(workflowUserManager, farmerUsername, adminUsername, () -> {
            try {
                Map<String, String> variables = createWorkflowVariables(submittedFormId);
