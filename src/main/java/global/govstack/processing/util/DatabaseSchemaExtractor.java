...
public class DatabaseSchemaExtractor {
    ...
    private List<String> findFormTables(Connection conn, DatabaseMetaData metadata) throws SQLException {
        ...
        // First, try to get tables from INFORMATION_SCHEMA
        String query = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES " +
                      "WHERE TABLE_SCHEMA = ? AND " +
                      "(TABLE_NAME LIKE 'app_fd_%' OR " +
                      " TABLE_NAME LIKE 'farmer%' OR " +
                      " TABLE_NAME LIKE 'farm_%' OR " +
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            try (ResultSet rs = metadata.getTables(conn.getCatalog(), null, "%", new String[]{"TABLE"})) {
                while (rs.next()) {
                    String tableName = rs.getString("TABLE_NAME");
                    // Filter for likely form tables
                    if (tableName.startsWith("app_fd_") ||
                        tableName.contains("farmer") ||
                        tableName.contains("farm") ||
                        tableName.contains("registry")) {
                        tables.add(tableName);
